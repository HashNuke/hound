defmodule Hound.Helpers.Window do
  ...
  def current_window_handle do
    ...
    delegate_to_module driver_info[:driver_type], "Window", :current_window_handle
  end
  ...
  def window_handles do
    {:ok, driver_info} = Hound.driver_info
    delegate_to_module driver_info[:driver_type], "Window", :window_handles
  end
  ...
  def set_window_size(window_handle, width, height) do
    {:ok, driver_info} = Hound.driver_info
    delegate_to_module driver_info[:driver_type], "Window", :set_window_size, [window_handle, width, height]
  end
  ...
  def window_size(window_handle) do
    {:ok, driver_info} = Hound.driver_info
    delegate_to_module driver_info[:driver_type], "Window", :window_size, [window_handle]
  end
  ...
  def maximize_window(window_handle) do
    {:ok, driver_info} = Hound.driver_info
    delegate_to_module driver_info[:driver_type], "Window", :maximize_window, [window_handle]
  end
  ...
  def focus_frame(frame_id) do
    {:ok, driver_info} = Hound.driver_info
    delegate_to_module driver_info[:driver_type], "Window", :focus_frame, [frame_id]
  end
  ...
  def focus_parent_frame() do
    {:ok, driver_info} = Hound.driver_info
    delegate_to_module driver_info[:driver_type], "Window", :focus_parent_frame
  end
end
