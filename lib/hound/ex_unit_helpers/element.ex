defmodule Hound.ExUnitHelpers.Element do
  ...
  defmacro visible_text(element_id) do
    quote do
      visible_text(var!(meta)[:hound_connection], var!(meta)[:hound_session_id], unquote(element_id))
    end
  end
  ...
  defmacro set_value(element_id, input) do
    quote do
      set_value(var!(meta)[:hound_connection], var!(meta)[:hound_session_id], unquote(element_id), unquote(input))
    end
  end
  ...
  defmacro tag_name(element_id) do
    quote do
      tag_name(var!(meta)[:hound_connection], var!(meta)[:hound_session_id], unquote(element_id))
    end
  end
  ...
  defmacro clear_field(element_id) do
    quote do
      clear_field(var!(meta)[:hound_connection], var!(meta)[:hound_session_id], unquote(element_id))
    end
  end
  ...
  defmacro selected?(element_id) do
    quote do
      selected?(var!(meta)[:hound_connection], var!(meta)[:hound_session_id], unquote(element_id))
    end
  end
  ...
  defmacro enabled?(element_id) do
    quote do
      enabled?(var!(meta)[:hound_connection], var!(meta)[:hound_session_id], unquote(element_id))
    end
  end
